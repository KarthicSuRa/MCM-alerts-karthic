...
import { Theme, Notification, Severity, NotificationStatus, SystemStatusData, Session, Comment, NotificationUpdatePayload, Topic } from './types';
    ...
    if (session) {
      ...
      const notificationChannel = supabase
        ...
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'comments' }, async (payload) => {
             const newComment = {
                ...payload.new,
                user_email: payload.new.user_id === session.user.id ? (session.user.email ?? 'Current User') : 'Another User'
             } as Comment;
      ...
      const subscriptionChannel = supabase
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'topic_subscriptions', filter: `user_id=eq.${session.user.id}` }, (payload) => {
             if (payload.eventType === 'INSERT') {
                const newSub = payload.new as { topic_id: string; id: number };
                setTopics(prev => prev.map(t => t.id === newSub.topic_id ? {...t, subscribed: true, subscription_id: newSub.id} : t));
             }
             if (payload.eventType === 'DELETE') {
                 const oldSub = payload.old as { topic_id: string };
    ...
    if (error) {
        ...
        if (window.Notification && Notification.permission === 'granted') {
            new window.Notification('New MCM Alert', {
                ...
                icon: 'https://i.imgur.com/4sODJ2F.png' 
            });
        }
    }
